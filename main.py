...
async def handle_upload(
    ...
    try:
        ...
        if aspect_ratio == "vertical":
            w, h = short_clip.size
            target_w = int(h * 9 / 16)
            short_clip = short_clip.cropped(x1=(w/2)-(target_w/2), y1=0, x2=(w/2)+(target_w/2), y2=h)
        ...
        short_clip.write_videofile(f"processed_videos/{output_filename}", codec="libx264")
